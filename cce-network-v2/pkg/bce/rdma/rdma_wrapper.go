...
func (n *rdmaNetResourceSetWrapper) calculateLimiter(scopeLog *logrus.Entry) (limit.RdmaIPResourceManager, error) {
	scopeLog = scopeLog.WithField("nodeName", n.k8sObj.Name).WithField("method", "generateRdmaIPResourceManager")
	client := k8s.WatcherClient()
	if client == nil {
		scopeLog.Fatal("K8s client is nil")
	}
	k8sNode, err := client.Informers.Core().V1().Nodes().Lister().Get(n.k8sObj.OwnerReferences[0].Name)
	if err != nil {
	...
	vifFeatures := n.bceRDMANetResourceSet.k8sObj.Annotations[k8s.AnnotationRDMAInfoVifFeatures]
	rdmaResourceManger := limit.NewRdmaIPResourceManager(client, vifFeatures, n.k8sObj.Spec.ENI.PreAllocateENI, k8sNode, bccInstance.CpuCount, bccInstance.MemoryCapacityInGB)
	n.rdmaCapacity = rdmaResourceManger.CalaculateCapacity()
	return rdmaResourceManger, nil
}
...
func (n *rdmaNetResourceSetWrapper) prepareIPAllocation(scopedLog *logrus.Entry) (a *ipam.AllocationAction, err error) {
	...
	if n.rdmaCapacity != nil {
		n.manager.ForeachInstance(n.instanceID, n.k8sObj.Name, func(instanceID, interfaceID string, iface ipamTypes.InterfaceRevision) error {
			...
			findEni = true
			allocation.AvailableForAllocationIPv4 = n.rdmaCapacity.MaxRdmaIPPerRdmaInt - len(e.Spec.PrivateIPSet)
...
func (*rdmaNetResourceSetWrapper) getMaximumAllocatable(rdmaCapacity *limit.NodeRdmaCapacity) int {
	return rdmaCapacity.MaxRdmaIPPerRdmaInt - 1
}
