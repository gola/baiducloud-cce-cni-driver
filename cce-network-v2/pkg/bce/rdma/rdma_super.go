...
type realNetResourceSetInf interface {
	calculateLimiter(scopeLog *logrus.Entry) (limit.RdmaIPResourceManager, error)
	...
	getMaximumAllocatable(capacity *limit.NodeRdmaCapacity) int
...
type bceRDMANetResourceSet struct {
	...
	// limit
	rdmaCapacity *limit.NodeRdmaCapacity
...
func (n *bceRDMANetResourceSet) calculateLimiter() *limit.NodeRdmaCapacity {
	n.limiterLock.Lock()
	defer n.limiterLock.Unlock()

	// faset path to claculate limiter
	if isSetLimiter(n) {
		return n.rdmaCapacity
	}
	...
	_, err := n.real.calculateLimiter(scopedLog)
	if err != nil {
		scopedLog.Errorf("Calculate limiter failed: %v", err)
		return n.rdmaCapacity
	}

	scopedLog.WithField("limiter", logfields.Json(n.rdmaCapacity)).
		Info("override RDMA capacity to NetResourceSet success")
	return n.rdmaCapacity
}
...
func isSetLimiter(n *bceRDMANetResourceSet) bool {
	if n.rdmaCapacity != nil {
		return true
	}
	if n.k8sObj.Labels != nil && n.k8sObj.Labels[k8s.LabelIPResourceCapacitySynced] != "" {
		n.rdmaCapacity = &limit.NodeRdmaCapacity{
			MaxRdmaIPPerRdmaInt: n.k8sObj.Spec.ENI.MaxIPsPerENI,
		}
		// use customer max ip nums if defiend
		if operatorOption.Config.BCECustomerMaxRdmaIP != 0 && operatorOption.Config.BCECustomerMaxRdmaIP != n.rdmaCapacity.MaxRdmaIPPerRdmaInt {
			return false
		}
		return true
	}
...
func (n *bceRDMANetResourceSet) AllocateIPs(ctx context.Context, allocation *ipam.AllocationAction) error {
	...
	effectiveLimits := limiter.MaxRdmaIPPerRdmaInt
